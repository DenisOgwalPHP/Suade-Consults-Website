            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
				...
				<tr>
                  ...
                  <th>Supplier</th>
				  <th>Category</th>
				  <th style='overflow-x:scroll; overflow-y:scroll'>Short Description</th>
                  <th>Description</th>
                  <th>Cost</th>
				  <th>Availability</th>
				  <th>Featured</th>
               ...
               <?php
				...
				$sales6 ="SELECT ProductName,RegistrationDate,Supplier,Category,ShortDescription,Description,Cost,Availability,Approval,Featured,IDDesc FROM `productdetails` order by IDDesc Desc";
				...
				while($row5=mysqli_fetch_assoc($result5)){
				 ...
				 echo "<td>".$row5['ShortDescription']."</td>";
				 echo '<td style="overflow-x:scroll; overflow-y:scroll">'.$row5["Description"].'</td>';
				 ...
				 $Availability=$row5['Availability'];
				  ...
				  echo'<td><form method="get" ><a href="ProductsTable.php"><button type="submit" name="notavialable"  value="'.$id.'" class="btn btn-block btn-warning">Not Available</button></a></form></td>'; 
					...
					$Approval=$row5['Featured'];
				  $id=$row5['IDDesc'];
					if($Approval=='No'){
				  echo'<td><form method="get" ><a href="ProductsTable.php"><button type="submit" name="approve"  value="'.$id.'" class="btn btn-block btn-warning">Not Featured</button></a></form></td>'; 
					}else{				  
				  echo'<td><form method="get" ><a href="ProductsTable.php"><button type="submit" name="approvess"  value="'.$id.'" class="btn btn-block btn-warning">Featured</button></a></form></td>'; 
					}
				...
				$id=$_GET['approve'];
				if(isset($id)){
				$sales6 ="UPDATE  productdetails SET Featured='Yes' WHERE IDDesc='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
					echo '<script type="application/javascript">';
				...
				?>
				<?php
				if(isset($_GET['approvess'])){	
				$id=$_GET['approvess'];
				if(isset($id)){
				$sales6 ="UPDATE  productdetails SET Featured='No' WHERE IDDesc='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
					echo '<script type="application/javascript">';
				echo'alert("Product Successful");';
