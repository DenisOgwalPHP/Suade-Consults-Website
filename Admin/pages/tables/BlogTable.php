...
<head>
  ...
  <title>Suade Consults | Blog </title>
        ...
        <div class="col-12">
          ...
          <div class="card" style='overflow-x:scroll'>
            <div class="card-header">
              <h3 class="card-title">Blog Records</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               <?php
				...
				$sales6 ="SELECT * FROM `blogs` order by blogid Desc";
				$result5=mysqli_query($link,$sales6);
				$counter1=1;
				while($row5=mysqli_fetch_assoc($result5)){
                 echo"<tr>";
				 echo "<td>".$row5['blogtitle']."</td>";
				 echo "<td>".$row5['postedby']."</td>";
                 echo "<td>".$row5['blogshotdescription']."</td>";
				 echo "<td>".$row5['description']."</td>";
				 echo "<td>".$row5['Date']."</td>";
				  $id=$row5['blogid'];
				 echo'<td><form method="get" ><a href="BlogTable.php"><button type="submit" name="delete"  value="'.$id.'" class="btn btn-block btn-danger">Delete</button></a></form></td>';  
                echo"</tr>";
				}
				?>
				<?php
				if(isset($_GET['delete'])){
					$id=$_GET['delete'];	
                        if(isset($id)){
				$sales6 ="DELETE  FROM blogs WHERE blogid='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
				echo '<script type="application/javascript">';
				echo'alert("Blog successfully Deleted");';
				echo'window.location.href="BlogTable.php";';
				echo '</script>';
							}else{
				echo '<script type="application/javascript">';
				echo'alert("Blog Delete Failed");';
				//echo'alert("'.$id.'");';
				echo'window.location.href="BlogTable.php";';
				echo '</script>';
								}
